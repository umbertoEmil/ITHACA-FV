...
int main(){
  ...
  auto gaussDens = std::make_shared<muq::Modeling::Gaussian>(mu, muq::Modeling::Gaussian::DiagCovariance)->AsDensity();
  ...
  auto varDens = std::make_shared<muq::Modeling::InverseGamma>(alpha,beta)->AsDensity();
  ...
  auto problem = std::make_shared<muq::SamplingAlgorithms::SamplingProblem>(jointDens);
