...
{
public:
  appendVectors(int inputSize) : muq::Modeling::ModPiece(1*Eigen::VectorXi::Ones(inputSize),    // inputSizes = [1,1]
...
Eigen::VectorXd expList(double first, double last, Eigen::DenseIndex n)
{
    ...
    for (Eigen::DenseIndex i = 1; i < n; i++) // DenseIndex is just a typedef ptrdiff_t from the Eigen library
...
int main(int argc, char* argv[])
{
    ...
    M_Assert(TCrows > 0 && TCcols > 0, "Set up TCrows and TCcols with the rows and cols of the thermocouples"); 

    const unsigned int Nsamples = para->ITHACAdict->lookupOrDefault<int>("MCMCsamples", 0); 
    const unsigned int BurnIn = para->ITHACAdict->lookupOrDefault<int>("MCMCBurnIn", 0); 
    const unsigned int printLevel = para->ITHACAdict->lookupOrDefault<int>("printLevel", 0); 
    double measurementsStdDevPerCent = para->ITHACAdict->lookupOrDefault<double>("measurementsStdDev", 0);
    double regularizationParameter = para->ITHACAdict->lookupOrDefault<double>("regularizationParameter", 0);
    double ProposalVariance = para->ITHACAdict->lookupOrDefault<double>("ProposalVariance", 0);
    ...
    //Tests to perform
    double alphaMin = para->ITHACAdict->lookupOrDefault<double>("regularizationParameterMin", 0);
    double alphaMax = para->ITHACAdict->lookupOrDefault<double>("regularizationParameterMax", 0); 
    int alphaSize = para->ITHACAdict->lookupOrDefault<int>("regularizationParameterSamples", 0);
    bool GibbsSingleRun = para->ITHACAdict->lookupOrDefault<bool>("GibbsSingleRun", 0);
    ...
    bool proposalVarianceTest = para->ITHACAdict->lookupOrDefault<bool>("proposalVarianceTest", 0);
    bool regularizationParameterTest = para->ITHACAdict->lookupOrDefault<bool>("regularizationParameterTest", 0);
    ...
    for (label i = 0; i < T_true.internalField().size(); i++)
    {
        ...
        T_true.ref()[i] = example.a * cx * cx + example.b * cx * cy + example.c * cy - example.a * cz * cz + example.c;
    }
    ...
    auto measErrorDist = std::make_shared<muq::Modeling::Gaussian>(Eigen::VectorXd::Zero(TmeasTrue.size()), measurementsStdDev * measurementsStdDev * Eigen::MatrixXd::Identity(TmeasTrue.size(), TmeasTrue.size()));
    ...
    M_Assert(TCrows * TCcols == example.Tmeas.size(), "Control number of thermocouples rows and columns");
	...
	//unsigned int AdaptSteps = 1000;
        ...
        pt.put("Kernel1.MyProposal.ProposalVariance", ProposalVariance); // Variance of the isotropic MH proposal
        ...
        bayesianAnalyticalBenchmark bayesianExample(example, measurementsStdDev, regularizationParameter, TCrows, TCcols, pt); 

        // starting point
        std::vector<Eigen::VectorXd> start(1);
        start.at(0) = Eigen::VectorXd::Zero(bayesianExample.Nweights);

        std::shared_ptr<muq::SamplingAlgorithms::SampleCollection> samps = bayesianExample.Run(start);

        Info << "Post processing \n" << endl;
        Eigen::VectorXd sampMean = samps->Mean();
        std::cout << "Sample Mean = \n" << sampMean.transpose() << std::endl;
        std::cout << "Theta * Sample Mean = \n" << (example.Theta * sampMean).transpose() << std::endl;
	std::cout << "example.addSol + example.Tmeas = \n" << (example.addSol + example.Tmeas).transpose() <<std::endl;  
	...
	word MHMCMCregParResultsFolder = "./ITHACAoutput/MHMCMCpropVarTestResults/";
        ...
        for(int alphaI = 0; alphaI < alphaSize; alphaI++)
        {
            ...
            pt.put("Kernel1.Proposal", "MyProposal"); // Name of block defining the proposal distribution
            pt.put("Kernel1.MyProposal.Method", "MHProposal"); // Name of proposal class
            pt.put("Kernel1.MyProposal.ProposalVariance", alpha(alphaI)); // Variance of the isotropic MH proposal
	...
	word MHMCMCregParResultsFolder = "./ITHACAoutput/MHMCMCregParTestResults/";
	    ...
	    Info << "\n\nIteration " << alphaI + 1 << ", alpha = " << alpha(alphaI) << endl;
            auto bayesianExample = std::make_shared<bayesianAnalyticalBenchmark>(example, measurementsStdDev, alpha(alphaI), TCrows, TCcols, pt); 
...
//
   if(GibbsSingleRun) 
   {
       ...
       forAll(example.gWeights, kernelI)
       {
           ...
           pt.put("Kernel" + std::to_string(kernelI) + ".MyProposal.numBlocks", numBlocks);
           pt.put("Kernel" + std::to_string(kernelI) + ".MyProposal.sigma", measurementsStdDev);
           pt.put("Kernel" + std::to_string(kernelI) + ".MyProposal.alpha", regularizationParameter);
       }
       pt.put("KernelList", kernelsList); // the transition kernel
   ...
   if(MHMCMCRunOld) 
   {
	...
	std::shared_ptr<muq::Modeling::LinearOperator> ThetaOp = muq::Modeling::LinearOperator::Create(Theta);
	...
	Eigen::MatrixXd likelihoodCov = likelihoodStdDev * likelihoodStdDev * Eigen::MatrixXd::Identity(example.Tmeas.size(), example.Tmeas.size());
	
        auto likelihood = std::make_shared<muq::Modeling::Gaussian>(example.addSol + example.Tmeas, likelihoodCov)->AsDensity();

        Eigen::VectorXd priorMean = Eigen::VectorXd::Zero(example.gWeights.size());
	Eigen::MatrixXd priorPrecision = bayesianExample.MRFcorrelationMatrix * (2.0 * regularizationParameter / (likelihoodStdDev * likelihoodStdDev));
        auto prior = std::make_shared<muq::Modeling::Gaussian>(priorMean, priorPrecision, muq::Modeling::Gaussian::Mode::Precision )->AsDensity();
