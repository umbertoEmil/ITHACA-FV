...
void Fang2017filter::setTime(double _startTime, double _deltaTime, double _endTime)
{
...
void Fang2017filter::setObservationTime(int _observationStart, int _observationDelta)
{
...
void Fang2017filter::setInitialStateDensity(Eigen::VectorXd _mean, Eigen::MatrixXd _cov)
{
    if(stateSize == 0)
    {
...
void Fang2017filter::setParameterPriorDensity(Eigen::VectorXd _mean, Eigen::MatrixXd _cov)
{
    if(parameterSize == 0)
    {
...
Eigen::MatrixXd Fang2017filter::ensembleFromDensity(std::shared_ptr<muq::Modeling::Gaussian> _density)
{
...
void Fang2017filter::updateJointEns(Eigen::VectorXd _observation)
{
    ...
    jointEns.assignSamples(jointEns.getSamples() + crossCov * autoCovInverse * temp);
}
