...
void sequentialIHTP_linear::setDiffusivity(scalar _diff)
{
    diffusivity = _diff;
}

void sequentialIHTP_linear::setSpaceBasis(word type,
...
void sequentialIHTP_linear::set_gParametrized(word spaceBaseFuncType,
...
void sequentialIHTP_linear::update_gParametrized(List<scalar> weights)
{
...
volScalarField sequentialIHTP_linear::list2Field(List<scalar> list,
        scalar innerField)
{
    volScalarField& T = _T();
    fvMesh& mesh = _mesh();
...
Eigen::VectorXd  sequentialIHTP_linear::TSVD(Eigen::MatrixXd A,
...
void sequentialIHTP_linear::solveDirectOffline()
{
    restartOffline();
...
void sequentialIHTP_linear::parameterizedBCoffline(bool force)
{
    fvMesh& mesh = _mesh();
    Tbasis.resize(0);
...
label sequentialIHTP_linear::realTimeStep(int timeI)
{
    return timeI + NtimeStepsBetweenSamples * timeSampleI + 1;
}

void sequentialIHTP_linear::reconstrucT(word outputFolder)
{
...
void sequentialIHTP_linear::parameterizedBC(word outputFolder, volScalarField initialField,
        word linSys_solver,
        label TSVD_filter)
{
...
void sequentialIHTP_linear::set_valueFraction()
{
    fvMesh& mesh = _mesh();
...
void sequentialIHTP_linear::assignDirectBC(label timeI)
{
    fvMesh& mesh = _mesh();
    volScalarField& T = _T();
    set_valueFraction();
...
void sequentialIHTP_linear::solveT0(volScalarField initialField)
{
...
void sequentialIHTP_linear::solveAdditional()
{
...
void sequentialIHTP_linear::solveDirect(label _offline)
{
    if(_offline)
    {
        restartOffline();
...
void sequentialIHTP_linear::readThermocouples()
{
...
Eigen::VectorXd sequentialIHTP_linear::fieldValueAtThermocouples(
    ...
    PtrList<volScalarField> fieldList)
{
    ...
    {
        Info << "The input fieldList of sequentialIHTP_linear::fieldValueAtThermocouples can have size Ntimes + 1 (=" << Ntimes + 1 << ") or\n";
...
void sequentialIHTP_linear::restart()
{
    Time& runTime = _runTime();
...
void sequentialIHTP_linear::restartOffline()
{
...
void sequentialIHTP_linear::restartT0()
{
    Info << "Setting endTime to offlineEndTime" << endl;
    restart();
    Time& runTime = _runTime();
...
void sequentialIHTP_linear::sampling2symulationTime()
{
...
void sequentialIHTP_linear::parameterizedBC_postProcess(
