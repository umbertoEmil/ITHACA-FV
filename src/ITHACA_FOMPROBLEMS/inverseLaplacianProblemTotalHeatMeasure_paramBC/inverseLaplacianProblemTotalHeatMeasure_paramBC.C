...
inverseLaplacianProblemTotalHeatMeasure_paramBC::inverseLaplacianProblemTotalHeatMeasure_paramBC(int argc, char* argv[])
    :
...
void inverseLaplacianProblemTotalHeatMeasure_paramBC::parameterizedBCoffline(bool force)
{
    fvMesh& mesh = _mesh();
    Tbasis.resize(0);
    Tad_base.resize(0);
...
Eigen::VectorXd inverseLaplacianProblemTotalHeatMeasure_paramBC::parameterizedBC(word linSys_solver,
        double regPar)
{
    ...
    linSys[1] = gIntegralWeight * gIntegral_meas * phi + Theta.transpose() * (Tmeas + addSol);
