...
inverseLaplacianProblemTotalHeatMeasure_CG::inverseLaplacianProblemTotalHeatMeasure_CG(int argc, char* argv[])
    :
...
void inverseLaplacianProblemTotalHeatMeasure_CG::computeGradJ()
{
    ...
    {
        gradJ [faceI] = - lambda.boundaryField()[hotSide_ind][faceI]  + gIntegralWeight * ( gIntegral - gIntegral_meas );
...
void inverseLaplacianProblemTotalHeatMeasure_CG::computeSearchStep()
{
    ...
    beta = Tdiff.dot(Tsens) - gIntegralWeight * Pintegral * (gIntegral - gIntegral_meas);
...
{
    double Jold = J;
    J = 0.5 * Tdiff.dot(Tdiff) + 0.5 * gIntegralWeight * (gIntegral - gIntegral_meas) * (gIntegral - gIntegral_meas);
