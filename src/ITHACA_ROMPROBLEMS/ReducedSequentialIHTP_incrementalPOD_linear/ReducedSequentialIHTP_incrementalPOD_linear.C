...
/// \file
/// Source file of the reducedSequentialIHTP_incrementalPOD_linear class
...
// Constructor
reducedSequentialIHTP_incrementalPOD_linear::reducedSequentialIHTP_incrementalPOD_linear()
{
}
...
void reducedSequentialIHTP_incrementalPOD_linear::computeHeatFluxWeights(
...
{
    timeSampleI = 0;
    scalar onlineCPUtime_FOM_avg = 0;
    scalar onlineCPUtime_ROM_avg = 0;
    ...
    while(timeSampleI < timeSamplesNum)
    {
        ...
        List<scalar> temp_Wold = heatFluxWeightsOld;
        List<scalar> temp_gradOld = heatFluxWeightsOld;
                    ...
                    T_ic_projectionTol)
            {
                if(previousWasReduced)
                {
                    ...
                    ITHACAutilities::assignIF(Ttime[NtimeStepsBetweenSamples -1], 
                            _initialField);
                    heatFluxWeightsOld = temp_Wold;
                    heatFluxWeightsGrad_old = temp_gradOld;
                }
            }
            else
            {
        ...
        if(doOnline)
        {
            onlineCPUtime_ROM_avg += elapsed_time_ms;
        }
        else
        {
            onlineCPUtime_FOM_avg += elapsed_time_ms;
        }
        Info << "CPU time = " << elapsed_time_ms << " milliseconds" << endl << endl;

        timeSampleI++;
    }
    onlineCPUtime_FOM_avg = onlineCPUtime_FOM_avg / timeSamplesNum;
    onlineCPUtime_ROM_avg = onlineCPUtime_ROM_avg / timeSamplesNum;
    Eigen::VectorXd onlineCPUtime_eig(1);
    onlineCPUtime_eig(0) = onlineCPUtime_FOM_avg;
    ITHACAstream::exportMatrix(
                    onlineCPUtime_eig, "onlineCPUtime_FOM_avg", "eigen", "./");
    onlineCPUtime_eig(0) = onlineCPUtime_ROM_avg;
    ITHACAstream::exportMatrix(
                    onlineCPUtime_eig, "onlineCPUtime_ROM_avg", "eigen", "./");
...
void reducedSequentialIHTP_incrementalPOD_linear::solveT_ic_online(
...
double reducedSequentialIHTP_incrementalPOD_linear::computeT_ic_projectionError(
...
void reducedSequentialIHTP_incrementalPOD_linear::findMagicPoints()
{
    magicPoints.clear();
...
void reducedSequentialIHTP_incrementalPOD_linear::projectT_ic()
{
    Info << "\n*****************************************************" << endl;
...
void reducedSequentialIHTP_incrementalPOD_linear::projectDirectOntoT_ic()
{
...
void reducedSequentialIHTP_incrementalPOD_linear::projectionErrorOffline()
{
...
void reducedSequentialIHTP_incrementalPOD_linear::pointProjectionOffline()
{
