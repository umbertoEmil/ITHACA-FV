...
heatFluxRelErr_L2 = np.flip(heatFluxRelErr_L2, 0)

heatFluxRelErr_Linf = np.reshape(heatFluxRelErr_Linf, (deltaX.size, deltaT.size))
heatFluxRelErr_Linf = np.flip(heatFluxRelErr_Linf, 0)
...
j = len(deltaX)
for dx in deltaX:
    plt.loglog(deltaT, heatFluxRelErr_L2[i,:],'-o', label = "Mesh " + str(j), markersize = 15)
    i = i + 1
    j = j - 1
