...
###############################################################
forderTSVD = "./TSVD/regParameter10/"
noiseLevel = np.loadtxt(forderTSVD + "noiseLevel.txt")

heatFluxRelErr_L2 = np.loadtxt(forderTSVD + "heatFluxRelErr_L2_max_list.txt")
...
for i in range(len(noiseLevel)):
    ...
    relErr_L2normMin[i] = np.quantile(vec, 0.1)
    relErr_L2normmax[i] = np.quantile(vec, 0.9)
plt.errorbar(noiseLevel, relErr_L2norm, yerr=[relErr_L2norm - relErr_L2normMin, relErr_L2normmax - relErr_L2norm], markersize=15,fmt='ks--', capsize=12, label = r'TSVD, $\alpha_{TSVD} = 10$')

###############################################################
forderTSVD = "./TSVD/regParameter8/"
noiseLevel = np.loadtxt(forderTSVD + "noiseLevel.txt")
...
for i in range(len(noiseLevel)):
    ...
    relErr_L2normMin[i] = np.quantile(vec, 0.1)
    relErr_L2normmax[i] = np.quantile(vec, 0.9)
plt.errorbar(noiseLevel, relErr_L2norm, yerr=[relErr_L2norm - relErr_L2normMin, relErr_L2normmax - relErr_L2norm], markersize=15,fmt='g^-.', capsize=12, label = r'TSVD, $\alpha_{TSVD} = 8$')



plt.xlabel('Noise standard dev.')
plt.ylabel(r'$max_{t\in(0,t_f]} ||e_{rel}||_{L^2(\Gamma_{s_{in}})} $')
