...
diffusivity = 0.033333333333333
CFL = deltaT * diffusivity * 3 / (edgeLength * edgeLength)
print deltaT
print relErr_L2
print Ncells
print"CFL = "
...
ax1.plot(edgeLength, relErr_L2[0,:], "bo", markersize=15, label=r"$|| \epsilon ||_{L^2(\Omega_s \times (0, t_f])}$")
ax1.plot(edgeLength, relErr_Linf[0,:], "kv", markersize=15,label=r"$|| \epsilon ||_{L^\infty(\Omega_s \times (0, t_f])}$") 
...
#labels[0] = Text(0.01,1,u'$\\mathdefault{0.8}$') 
#print labels
#
#ax2.set_xticklabels(labels)
ax2.set_xticklabels(['0.8', '0.45', '0.2' ,'0.05']) 
...
#ax1 = plt.axes(xscale='log', yscale='log')
#
#ax1.plot(deltaT, relErr_L2[:,0], "bo", markersize=15, label=r"$|| \epsilon ||_{L^2(\Omega_s \times (0, t_f])}$")
#ax1.plot(deltaT, relErr_Linf[:,0], "kv", markersize=15,label=r"$|| \epsilon ||_{L^\infty(\Omega_s \times (0, t_f])}$") 
#
#
##plt.title(r"$Mesh = $", fontsize=25)
#plt.xlabel(r'$\Delta t$ [$m$]', fontsize=25)
#plt.minorticks_on()
#plt.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
#plt.legend(loc='best', fontsize=25)
#
...
#ax2.set_xlim(ax1.get_xlim())
#plt.xscale('log')
##ax2.set_xticks(deltaT,  minor=True)
