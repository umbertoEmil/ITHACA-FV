...
{
    public:
        appendVectors() : muq::Modeling::ModPiece(1 * Eigen::VectorXi::Ones(
                        2),  // inputSizes = [1,1]
...
int main()
{
    ...
    std::vector<std::string> kernelNames = muq::Utilities::StringUtilities::Split(
            kernelString, ',');
    ...
    auto mcmc = std::make_shared<muq::SamplingAlgorithms::SingleChainMCMC>(pt,
                problem);
