...
int main(int argc, char* argv[])
{
    ...
    std::cout << "We observe the state x by mean of the observation matrix \nH = \n"
              << H << std::endl;
    std::cout <<
              "The objective is to reconstruct the vector b knowing H, x_0, and A" <<
              std::endl;
    int Ntimes = 51;
    ...
    for (int timeI = 0; timeI < Ntimes - 1; timeI++)
    {
        ...
        for (int i = 0; i < Nseeds; i++)
        {
            ...
            {
                forwardSamples.col(i) = A * forwardSamplesOld.col(i) +
                                        priorDensity->Sample();//priorSamples.col(i);
            }
        }
        ...
        if (sampleFlag == 0)
        {
            ...
            //Kalman filter
            posteriorSamples = ITHACAmuq::muq2ithaca::EnsembleKalmanFilter(priorSamples,
    ...
    ITHACAstream::exportMatrix(maxConfidence, "maxConfidence", "eigen",
                               outputFolder);
    ITHACAstream::exportMatrix(singleVariableSamples, "singleVariableSamples",
                               "eigen", outputFolder);
    return 0;
}
